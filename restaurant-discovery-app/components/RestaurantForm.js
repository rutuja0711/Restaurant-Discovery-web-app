...
export default function RestaurantForm({ initialData, restaurantId }) {
  ...
  async function handleAddressBlur() {
    if (form.address && form.location) {
      const coords = await geocodeInBrowser(`${form.address}, ${form.location}`);
      if (coords) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      console.log('PAYLOAD BEING SENT:', payload.latitude, payload.longitude);
      ...
      <form className="wizard-panel" onSubmit={handleSubmit}>
        {stepIndex === 0 && (
          <>
            ...
            <input placeholder="Location / city (e.g. Nashik)" value={form.location} onChange={(e) => update('location', e.target.value)} onBlur={handleAddressBlur} />
            {errors.location && <p className="field-error">{errors.location}</p>}

            <input placeholder="Contact number" value={form.contactNumber} onChange={(e) => update('contactNumber', e.target.value)} />

            <label className="wizard-hint">Confirm the exact location on the map</label>
            <LocationPicker
